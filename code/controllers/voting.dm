			...
			switch(vote_type)
				...
				if("crew_transfer")
					if(check_rights(R_ADMIN, 0))
						...
						choices.Add("Initiate Crew Transfer", "Continue The Round")
				if("custom")
					question = html_encode_russian(input(usr,"What is the vote for?") as text|null)
					if(!question)	return 0
					for(var/i=1,i<=10,i++)
						var/option = capitalize(html_encode_russian(input(usr,"Please enter an option or hit cancel to finish") as text|null))
