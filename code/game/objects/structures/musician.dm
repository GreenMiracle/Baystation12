...
/obj/structure/device/piano/Topic(href, href_list)
	...
	if(href_list["newsong"])
		...
		if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
			...
			var/newline = html_encode_russian(input("Enter your line: ", "Piano") as text|null)
			if(!newline)
				return
			...
			var/content = html_encode_russian(input("Enter your line: ", "Piano", song.lines[num]) as text|null)
			if(!content)
				return
			...
			var/t = ""
			do
				t = html_encode_russian(input(usr, "Please paste the entire song, formatted:", text("[]", src.name), t)  as message)
				if (!in_range(src, usr))
					return
